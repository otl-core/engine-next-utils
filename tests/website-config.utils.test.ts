...
import type { SiteConfig } from "@otl-core/cms-types";
...
describe("Website Config Helpers", () => {
  ...
  describe("getWebsiteConfig", () => {
    it("should return website config from site config", async () => {
      mockFetchConfigs.mockResolvedValue(mockSiteConfig);
    ...
    it("should return null when config has no website property", async () => {
      mockFetchConfigs.mockResolvedValue({} as SiteConfig);
      const result = await getWebsiteConfig();
      expect(result).toBeNull();
    });
